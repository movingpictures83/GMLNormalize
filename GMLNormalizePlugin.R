...
#files = list.files(path = '.', pattern = ".graphml")
...
input <- function(inputfile) {
  ...
  nameOfNetwork <<- paste(prefix(),parameters["network", 2], sep="/")

  abundances <<- unlist(read.csv(paste(prefix(),parameters["meanabundance", 2],sep="/"),sep="\t"))
}  
...
output <- function(outputfile) {
  ...
  writeChar(networkO,outputfile,nchars = nchar(networkO))
  r = readBin(outputfile, raw(), file.info(nameOfNetwork)$size)
  r[r==as.raw(0)] = as.raw(0x20) ## replace with 0x20 = <space>
  writeBin(r, outputfile)
